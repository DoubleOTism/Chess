...
import com.chess.common.LocationMaker;
...
public class Rook extends AbstractPiece implements Moveable {
    ...
    private void getRankCandidates(
            ...
            int offset) {
        try {
            Location next = LocationMaker.build(current, current.getFile().ordinal(), offset);
            while (squareMap.containsKey(next)) {
                ...
                moveCandidates.add(next);
                next = LocationMaker.build(next, next.getFile().ordinal(), offset);
            }
        } catch (Exception e) {
    ...
    private void getFileCandidates(
            ...
            int offset) {
        try {
            Location next = LocationMaker.build(current, offset, 0);
            while (squareMap.containsKey(next)) {
                if (squareMap.get(next).isOccupied()) {
                    if (squareMap.get(next).getCurrentPiece().pieceColor.equals(this.pieceColor)) {
                        break;
                ...
                moveCandidates.add(next);
                next = LocationMaker.build(next, offset, 0);
            }
        } catch (Exception e) { }
    }
